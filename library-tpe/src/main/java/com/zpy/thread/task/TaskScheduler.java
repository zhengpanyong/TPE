...
public final class TaskScheduler {
    ...
    private final int CORE_POOL_SIZE = CPU_COUNT * 2 + 1 < 10? 10:CPU_COUNT * 2 + 1;//常驻最大线程数
        ...
        if (task.groupName == IGroupedTask.DEFAULT_GROUP_NAME) {
            ...
            if (oldTask != null) {
                ...
                if (dualPolicy == IGroupedTask.DISCARD_NEW) {//2.如果同一个组的相同任务已经被提交，则丢弃该任务
                    ...
                    oldTask = taskPriorityManager.getOldTaskNotRunning(task);
                    if (oldTask != null && oldTask.getStatus() != ITask.STATUS_RUNNING) {
                        oldTask.stop();
                    }
                    if (TPE.isDebug()) {
